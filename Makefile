...
$(DOCKER_PHP_CONTAINER) = $(DOCKER_PHP_CONTAINER:php)
$(DOCKER_NODE_CONTAINER) = $(DOCKER_NODE_CONTAINER:node)

EXEC_PHP        = $(DOCKER_COMPOSE) exec -T $(DOCKER_PHP_CONTAINER) /entrypoint
EXEC_JS         = $(DOCKER_COMPOSE) exec -T $(DOCKER_NODE_CONTAINER) /entrypoint
...
no-docker: ## Don't use commands inside docker container
	$(eval DOCKER_COMPOSE := \#)
	$(eval EXEC_PHP := )
	$(eval EXEC_JS := )
